...
@Component({
  selector: 'app-afficheliste-depot-recycleur',
  standalone: true,
  imports: [
    RowComponent,
    CommonModule,
  ...
  templateUrl: './afficheliste-depot-recycleur.component.html',
  styleUrl: './afficheliste-depot-recycleur.component.scss',
})
export class AffichelisteDepotRecycleurComponent {
  constructor(
  ...
  DePotLists() {
    ...
    this.depotService.getDepots(currentUser.id).subscribe((res: any) => {
      ...
      this.depots.forEach((depot: Depot) => {
        this.checkDepotConteneurTransformerrecycleur(depot);
        console.log(depot);
      });
    });
  }

  checkDepotConteneurTransformerrecycleur(depot: any): void {
    const nom = depot.nom;
    const lieu = depot.lieu;
    console.log(depot.nom);
    console.log(depot.lieu);
    this.depotService.checkDepotConteneurTransformerrecycleur({nom,lieu}).subscribe(
      (response: any) => {
