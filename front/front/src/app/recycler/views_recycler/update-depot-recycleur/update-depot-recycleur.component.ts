...
@Component({
  selector: 'app-update-depot-recycleur',
  standalone: true,
  ...
  templateUrl: './update-depot-recycleur.component.html',
  styleUrl: './update-depot-recycleur.component.scss'
})
export class UpdateDepotRecycleurComponent {
  depotId!: any;
  depot: Depot = {
    id: 0,
    nom: '',
    ...
    if (this.depotId) {
      this.depotService.getDepot(this.depotId).subscribe({
        next: (res: any) => {
          ...
          this.depotService.checkDepotConteneurTransformerrecycleur(inputData).subscribe(
            (response: any) => {
